...
var SettingsTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Add New Theme").setDesc("Create a new theme from the current settings.").addText((text) => {
      newThemeName = text.setPlaceholder("New theme name").setValue(this.plugin.settings.newTheme).onChange((value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Codeblock Background Color").then((setting) => {
      this.createPickr(
        this.plugin,
        containerEl,
        setting,
        "codeblock_background",
        (relevantThemeColors) => relevantThemeColors[getCurrentMode()].codeblock.backgroundColor,
        (relevantThemeColors, saveColor) => {
          relevantThemeColors[getCurrentMode()].codeblock.backgroundColor = saveColor;
        }
      );
    });
    new import_obsidian.Setting(containerEl).setName("Codeblock Text Color").then((setting) => {
      this.createPickr(
        this.plugin,
        containerEl,
        setting,
        "codeblock_text",
        (relevantThemeColors) => relevantThemeColors[getCurrentMode()].codeblock.textColor,
        (relevantThemeColors, saveColor) => {
          relevantThemeColors[getCurrentMode()].codeblock.textColor = saveColor;
        }
      );
    });
    ...
    new import_obsidian.Setting(containerEl).setName("Gutter Background Color").then((setting) => {
      this.createPickr(
        this.plugin,
        containerEl,
        setting,
        "gutter_background",
        (relevantThemeColors) => relevantThemeColors[getCurrentMode()].gutter.backgroundColor,
        (relevantThemeColors, saveColor) => {
          relevantThemeColors[getCurrentMode()].gutter.backgroundColor = saveColor;
        },
    ...
    new import_obsidian.Setting(containerEl).setName("Line Number Color").then((setting) => {
      this.createPickr(
        this.plugin,
        containerEl,
        setting,
        "line_number",
        (relevantThemeColors) => relevantThemeColors[getCurrentMode()].gutter.textColor,
        (relevantThemeColors, saveColor) => {
          relevantThemeColors[getCurrentMode()].gutter.textColor = saveColor;
        },
        () => !this.plugin.settings.currentTheme.settings.codeblock.lineNumbers
      );
    });
    ...
    new import_obsidian.Setting(containerEl).setName("Highlight Line Numbers").setDesc("If enabled, highlights will also highlight the line numbers.").addToggle((toggle) => {
      let highlightLineNumbersToggle = toggle.setValue(this.plugin.settings.currentTheme.settings.gutter.highlight).setDisabled(!this.plugin.settings.currentTheme.settings.codeblock.lineNumbers).onChange((value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Indicate Current Line Number").setDesc("If enabled, the current line number in codeblocks will be indicated with a separate color.").setClass("code-styler-spaced").addToggle((toggle) => {
      let indicateCurrentLineNumberToggle = toggle.setValue(this.plugin.settings.currentTheme.settings.gutter.activeLine).setDisabled(!this.plugin.settings.currentTheme.settings.codeblock.lineNumbers).onChange((value) => {
      ...
      this.createPickr(
        ...
        setting,
        "active_line_number",
        (relevantThemeColors) => relevantThemeColors[getCurrentMode()].gutter.activeTextColor,
        (relevantThemeColors, saveColor) => {
          relevantThemeColors[getCurrentMode()].gutter.activeTextColor = saveColor;
        },
        () => !this.plugin.settings.currentTheme.settings.codeblock.lineNumbers
      );
    });
    containerEl.createEl("h4", { text: "Header Appearance" });
    new import_obsidian.Setting(containerEl).setName("Header Background Color").then((setting) => {
      this.createPickr(
        this.plugin,
        containerEl,
        setting,
        "header_background",
        (relevantThemeColors) => relevantThemeColors[getCurrentMode()].header.backgroundColor,
        (relevantThemeColors, saveColor) => {
          relevantThemeColors[getCurrentMode()].header.backgroundColor = saveColor;
        }
      );
    });
    ...
    new import_obsidian.Setting(containerEl).setName("Header Title Text Styling").setDesc("Style the header title text using bold and italic toggles, by setting a font or by setting a text color.").addToggle((toggle) => {
      ...
      text.setPlaceholder("Font").setValue(this.plugin.settings.currentTheme.settings.header.title.textFont).onChange((value) => {
      ...
      this.createPickr(
        ...
        setting,
        "header_title_text",
        (relevantThemeColors) => relevantThemeColors[getCurrentMode()].header.title.textColor,
        (relevantThemeColors, saveColor) => {
          relevantThemeColors[getCurrentMode()].header.title.textColor = saveColor;
        }
      );
    });
    new import_obsidian.Setting(containerEl).setName("Collapse Placeholder Text").setDesc("Title placeholder text for collapsed code when no title parameter is set.").addText((text) => text.setPlaceholder("Collapsed Code").setValue(this.plugin.settings.currentTheme.settings.header.collapsePlaceholder).onChange((value) => {
      this.plugin.settings.currentTheme.settings.header.collapsePlaceholder = value;
      (async () => {
        await this.plugin.saveSettings();
      })();
    }));
    new import_obsidian.Setting(containerEl).setName("Header Separator Color").setDesc("Color of the line separating the codeblock header and the codeblock.").then((setting) => {
      this.createPickr(
        this.plugin,
        containerEl,
        setting,
        "header_separator",
        (relevantThemeColors) => relevantThemeColors[getCurrentMode()].header.lineColor,
        (relevantThemeColors, saveColor) => {
          relevantThemeColors[getCurrentMode()].header.lineColor = saveColor;
        }
      );
    });
    ...
    new import_obsidian.Setting(containerEl).setName("Header Language Tag Background Color").then((setting) => {
      this.createPickr(
        this.plugin,
        containerEl,
        setting,
        "header_language_tag_background",
        (relevantThemeColors) => relevantThemeColors[getCurrentMode()].header.languageTag.backgroundColor,
        (relevantThemeColors, saveColor) => {
          relevantThemeColors[getCurrentMode()].header.languageTag.backgroundColor = saveColor;
        },
    ...
    new import_obsidian.Setting(containerEl).setName("Header Language Tag Text Styling").setDesc("Style the header language tag text using bold and italic toggles, by setting a font or by setting a text color.").addToggle((toggle) => {
      ...
      languageIconsFontText = text.setPlaceholder("Font").setValue(this.plugin.settings.currentTheme.settings.header.languageTag.textFont).setDisabled(this.plugin.settings.currentTheme.settings.header.languageTag.display === "none").onChange((value) => {
      ...
      this.createPickr(
        ...
        (relevantThemeColors) => relevantThemeColors[getCurrentMode()].header.languageTag.textColor,
        (relevantThemeColors, saveColor) => {
          relevantThemeColors[getCurrentMode()].header.languageTag.textColor = saveColor;
        },
        () => this.plugin.settings.currentTheme.settings.header.languageTag.display === "none"
      );
    });
    ...
    let languageIconsColoredToggle;
    new import_obsidian.Setting(containerEl).setName("Language Icons Colored").setDesc("If disabled, language icons will be black and white.").addToggle((toggle) => {
      languageIconsColoredToggle = toggle.setValue(this.plugin.settings.currentTheme.settings.header.languageIcon.displayColor).setDisabled(this.plugin.settings.currentTheme.settings.header.languageIcon.display === "none").onChange((value) => {
    ...
    new import_obsidian.Setting(containerEl).setName("Editor Active Line Highlight").setDesc("If enabled, highlights the active line outside codeblocks.").setClass("code-styler-spaced").addToggle((toggle) => {
      ...
      this.createPickr(
        ...
        (relevantThemeColors) => relevantThemeColors[getCurrentMode()].highlights.activeEditorLineColor,
        (relevantThemeColors, saveColor) => {
          relevantThemeColors[getCurrentMode()].highlights.activeEditorLineColor = saveColor;
        },
    ...
    new import_obsidian.Setting(containerEl).setName("Codeblock Active Line Highlight").setDesc("If enabled, highlights the active line inside codeblocks.").setClass("code-styler-spaced").addToggle((toggle) => {
      ...
      this.createPickr(
        ...
        (relevantThemeColors) => relevantThemeColors[getCurrentMode()].highlights.activeCodeblockLineColor,
        (relevantThemeColors, saveColor) => {
          relevantThemeColors[getCurrentMode()].highlights.activeCodeblockLineColor = saveColor;
        },
    ...
    new import_obsidian.Setting(containerEl).setName("Default Highlight Color").setDesc("Used by the 'hl' parameter.").then((setting) => {
      this.createPickr(
        this.plugin,
        containerEl,
        setting,
        "default_highlight",
        (relevantThemeColors) => relevantThemeColors[getCurrentMode()].highlights.defaultColor,
        (relevantThemeColors, saveColor) => {
          relevantThemeColors[getCurrentMode()].highlights.defaultColor = saveColor;
        }
      );
    });
    let newHighlightText;
    new import_obsidian.Setting(containerEl).setName("Add Alternative Highlight").setDesc("Define a new alternative highlight name. Colors can be modified after it is added.").addText((value) => {
      ...
      button.onClick(() => {
        if (this.plugin.settings.newHighlight.trim() === "")
          new import_obsidian.Notice("Please enter a color name.");
        else if (!/^[^\d]\w*$/.test(this.plugin.settings.newHighlight))
          new import_obsidian.Notice(`"${this.plugin.settings.newHighlight}" is not a valid color name.`);
    ...
    new import_obsidian.Setting(containerEl).setName("Inline Code Background Color").then((setting) => {
      this.createPickr(
        this.plugin,
        containerEl,
        setting,
        "inline_background",
        (relevantThemeColors) => relevantThemeColors[getCurrentMode()].inline.backgroundColor,
        (relevantThemeColors, saveColor) => {
          relevantThemeColors[getCurrentMode()].inline.backgroundColor = saveColor;
        }
      );
    });
    new import_obsidian.Setting(containerEl).setName("Inline Code Text Color").then((setting) => {
      this.createPickr(
        this.plugin,
        containerEl,
        setting,
        "inline_text",
        (relevantThemeColors) => relevantThemeColors[getCurrentMode()].inline.textColor,
        (relevantThemeColors, saveColor) => {
          relevantThemeColors[getCurrentMode()].inline.textColor = saveColor;
        }
      );
    });
    new import_obsidian.Setting(containerEl).setName("Inline Code Active Text Color").setDesc("The text color when editing inline code.").then((setting) => {
      this.createPickr(
        this.plugin,
        containerEl,
        setting,
        "inline_active_text",
        (relevantThemeColors) => relevantThemeColors[getCurrentMode()].inline.activeTextColor,
        (relevantThemeColors, saveColor) => {
          relevantThemeColors[getCurrentMode()].inline.activeTextColor = saveColor;
        }
      );
    });
    ...
    new import_obsidian.Setting(containerEl).setName("Button Color").setDesc("Used for UI buttons like the copy code button.").then((setting) => {
      this.createPickr(
        this.plugin,
        containerEl,
        setting,
        "default_highlight",
        (relevantThemeColors) => relevantThemeColors[getCurrentMode()].advanced.buttonColor,
        (relevantThemeColors, saveColor) => {
          relevantThemeColors[getCurrentMode()].advanced.buttonColor = saveColor;
        }
      );
    });
    new import_obsidian.Setting(containerEl).setName("Button Active Color").setDesc("Color buttons use when activated.").then((setting) => {
      this.createPickr(
        this.plugin,
        containerEl,
        setting,
        "default_highlight",
        (relevantThemeColors) => relevantThemeColors[getCurrentMode()].advanced.buttonActiveColor,
        (relevantThemeColors, saveColor) => {
          relevantThemeColors[getCurrentMode()].advanced.buttonActiveColor = saveColor;
        }
      );
    });
    new import_obsidian.Setting(containerEl).setName("Gradient Highlighting").setDesc("If enabled, highlights fade away to the right. The slider sets the gradient color stop as a percentage.").addToggle((toggle) => toggle.setValue(this.plugin.settings.currentTheme.settings.advanced.gradientHighlights).onChange((value) => {
      ...
      let resettableSlider;
      setting.addSlider((slider) => {
        resettableSlider = slider.setLimits(0, 100, 1).setValue(+this.plugin.settings.currentTheme.settings.advanced.gradientHighlightsColorStop.slice(0, -1)).setDisabled(!this.plugin.settings.currentTheme.settings.advanced.gradientHighlights).setDynamicTooltip().onChange((value) => {
      ...
      let resetButton;
      setting.addExtraButton((button) => {
        resetButton = button.setIcon("reset").setDisabled(!this.plugin.settings.currentTheme.settings.advanced.gradientHighlights).setTooltip("Restore default color stop").onClick(() => {
    ...
    new import_obsidian.Setting(containerEl).setName("Language Colored Borders").setDesc("If enabled, languages with icons display a left border with the color of the icon. The slider sets the width of the border.").addToggle((toggle) => toggle.setValue(this.plugin.settings.currentTheme.settings.advanced.languageBorderColor).onChange((value) => {
      this.plugin.settings.currentTheme.settings.advanced.languageBorderColor = value;
      this.disableableComponents["languageBorderColor"].forEach((component) => {
        component.setDisabled(!value);
      });
      (async () => {
        await this.plugin.saveSettings();
      ...
      let resettableSlider;
      setting.addSlider((slider) => {
        resettableSlider = slider.setLimits(0, 20, 1).setValue(this.plugin.settings.currentTheme.settings.advanced.languageBorderWidth).setDisabled(!this.plugin.settings.currentTheme.settings.advanced.languageBorderColor).setDynamicTooltip().onChange((value) => {
      ...
      let resetButton;
      setting.addExtraButton((button) => {
        resetButton = button.setIcon("reset").setDisabled(!this.plugin.settings.currentTheme.settings.advanced.languageBorderColor).setTooltip("Restore default color stop").onClick(() => {
    ...
    new import_obsidian.Setting(containerEl).setName("Redirect Language Settings").setDesc("Use this textbox to redirect specific language colors and icons as a JSON with language names as keys and either a color key, an icon key or both as the value for a given language. Colours should be passed as CSS colors and icons should be passed as a string of the inside of an svg element. This setting is theme independent.").setClass("code-styler-setting-text-area").addTextArea((textArea) => textArea.setValue(JSON.stringify(this.plugin.settings.redirectLanguages) === "{}" ? "" : JSON.stringify(this.plugin.settings.redirectLanguages, null, 4)).setPlaceholder(JSON.stringify({ toml: { color: "#012345", icon: LANGUAGE_ICONS_DATA["APL"] } }, null, 4)).onChange((value) => {
      if (value === "") {
  ...
  // Setting Creation
  createPickr(plugin, containerEl, setting, id, getRelevantThemeColor, saveRelevantThemeColor, disabled) {
    let pickr = new PickrResettable(plugin, containerEl, setting, getRelevantThemeColor, saveRelevantThemeColor);
    pickr.on("show", (color, instance) => {
      if (typeof disabled !== "undefined" && disabled())
        instance.hide();
    }).on("save", (color, instance) => {
      const savedColor = `#${color.toHEXA().toString().substring(1)}`;
      instance.hide();
    ...
    setting.addExtraButton((button) => {
      pickr.resetButton = button.setIcon("reset").setDisabled(typeof disabled !== "undefined" && disabled()).setTooltip("Restore default color").onClick(() => {
        pickr.resetColor();
      });
    });
    ...
    Object.keys(this.plugin.settings.currentTheme.colors.light.highlights.alternativeHighlights).forEach((alternativeHighlightName) => {
      new import_obsidian.Setting(alternativeHighlightsContainer).setName(alternativeHighlightName).setDesc(`To highlight lines with this highlight, use the ${alternativeHighlightName} parameter.`).then((setting) => {
        this.createPickr(
          ...
          (relevantThemeColors, saveColor) => {
            relevantThemeColors[getCurrentMode()].highlights.alternativeHighlights[alternativeHighlightName] = saveColor;
          }
        );
        setting.addExtraButton((button) => {
...
var PickrResettable = class extends import_pickr.default {
  constructor(plugin, containerEl, setting, getRelevantThemeColor, saveRelevantThemeColor, tooltip) {
    const settings = {
    ...
    this.resetColor = () => {
      const resetColor = getRelevantThemeColor(plugin.settings.themes[plugin.settings.selectedTheme].colors);
...
function styleThemeColors(themeColors) {
  return Object.keys(themeColors.light.highlights.alternativeHighlights).reduce((result, alternativeHighlight) => {
    return result + `
...
function getThemeColors(themeModeColors) {
  return Object.entries({
    ...
    ...Object.entries(themeModeColors.highlights.alternativeHighlights).reduce((result, [alternativeHighlight, color]) => {
      result[`${alternativeHighlight.replace(/\s+/g, "-").toLowerCase()}-highlight-color`] = color;
      return result;
    }, {})
  }).reduce((result, [cssVariable, color]) => {
...
function styleLanguageColors(themeSettings, redirectLanguages, currentTheme) {
    ...
    if (languageDisplayName in LANGUAGE_COLORS || languageName in redirectLanguages && "color" in redirectLanguages[languageName]) {
      result += `
				.language-${languageName} {
					--language-border-color: ${(_b = (_a = redirectLanguages == null ? void 0 : redirectLanguages[languageName]) == null ? void 0 : _a["color"]) != null ? _b : LANGUAGE_COLORS[languageDisplayName]};
      ...
      if ((_c = THEME_STYLES == null ? void 0 : THEME_STYLES[currentTheme]) == null ? void 0 : _c.border) {
        ...
        themeSettings.advanced.languageBorderColor ? themeSettings.advanced.languageBorderWidth + THEME_STYLES[currentTheme].border.size : 0}px;
					}`;
      }
    }
    return result;
...
async function parseCodeblockSource(codeSection, sourcePath, plugin) {
  ...
  function parseCodeblockSection(codeSection2) {
    ...
    if (codeSection2.length === 0)
      return;
    let openingCodeblockLine = getOpeningLine(codeSection2);
    if (!openingCodeblockLine)
      return;
    let openDelimiter = (_a = /^\s*(?:>\s*)*(```+).*$/.exec(openingCodeblockLine)) == null ? void 0 : _a[1];
    if (!openDelimiter)
      return;
    let openDelimiterIndex = codeSection2.indexOf(openingCodeblockLine);
    let closeDelimiterIndex = codeSection2.slice(openDelimiterIndex + 1).findIndex((line) => line.indexOf(openDelimiter) !== -1);
...
function parseParameterString(parameterString, codeblockParameters, theme) {
  if (parameterString.startsWith("title:")) {
    ...
    if (highlightMatch) {
      ...
      else if (highlightMatch[1] in theme.colors.light.highlights.alternativeHighlights)
...
function parseHighlightedLines(highlightedLinesString) {
  ...
  let lineNumbers = /* @__PURE__ */ new Set();
  let plainText = /* @__PURE__ */ new Set();
  let regularExpressions = /* @__PURE__ */ new Set();
...
// src/EditingView.ts
function createCodeMirrorExtensions(settings, languageIcons) {
  ...
  const codeblockLines = import_view.ViewPlugin.fromClass(
    class CodeblockLines {
      constructor(view) {
        ...
        this.view = view;
        this.decorations = import_state.RangeSet.empty;
        ...
        for (const codeblock of codeblocks) {
          ...
          (0, import_language.syntaxTree)(view.state).iterate({
            ...
            enter(syntaxNode) {
              ...
              if (startLine) {
                codeblockParameters = parseCodeblockParameters(trimParameterLine(lineText), settings2.currentTheme);
                excludedCodeblock = isExcluded(codeblockParameters.language, [settings2.excludedCodeblocks, settings2.excludedLanguages].join(",")) || codeblockParameters.ignore;
                lineNumber = 0;
              ...
              if (syntaxNode.type.name.includes("HyperMD-codeblock")) {
                decorations.push(import_view.Decoration.line({ attributes: { style: `--line-number-gutter-width: ${lineNumberMargin ? lineNumberMargin + "px" : "calc(var(--line-number-gutter-min-width) - 12px)"}`, class: (settings2.specialLanguages.some((regExp) => new RegExp(regExp).test(codeblockParameters.language)) || startLine || endLine ? "code-styler-line" : getLineClass(codeblockParameters, lineNumber, line.text).join(" ")) + (["^$"].concat(settings2.specialLanguages).some((regExp) => new RegExp(regExp).test(codeblockParameters.language)) ? "" : ` language-${codeblockParameters.language}`) } }).range(syntaxNode.from));
                decorations.push(import_view.Decoration.line({}).range(syntaxNode.from));
                decorations.push(import_view.Decoration.widget({ widget: new LineNumberWidget(lineNumber, codeblockParameters, maxLineNum, startLine || endLine) }).range(syntaxNode.from));
                lineNumber++;
              }
            }
          });
        }
        this.decorations = import_state.RangeSet.of(decorations, true);
      }
      destroy() {
          ...
          if (startLine) {
            ...
            if (!isExcluded(codeblockParameters.language, [settings.excludedCodeblocks, settings.excludedLanguages].join(",")) && !codeblockParameters.ignore)
              if (!settings.specialLanguages.some((regExp) => new RegExp(regExp).test(codeblockParameters.language)))
                builder.add(line.from, line.from, import_view.Decoration.widget({ widget: new HeaderWidget(codeblockParameters, settings.currentTheme.settings, languageIcons), block: true }));
              else
                continue;
          } else {
    ...
    create(state) {
      ...
      for (let i2 = 1; i2 < state.doc.lines; i2++) {
        ...
        if (collapseStart && collapseEnd) {
          builder.add(collapseStart.from, collapseEnd.to, import_view.Decoration.replace({ effect: collapse.of([import_view.Decoration.replace({ block: true }).range(collapseStart.from, collapseEnd.to)]), block: true, side: -1 }));
          collapseStart = null;
          collapseEnd = null;
        }
      }
  ...
  class LineNumberWidget extends import_view.WidgetType {
    constructor(lineNumber, codeblockParameters, maxLineNum, empty) {
      super();
  ...
  function collapseOnClick(view, target) {
    ...
    let folded = false;
    (_a = view.state.field(codeblockCollapse, false)) == null ? void 0 : _a.between(position, position, () => {
      folded = true;
    });
    let collapseStart = null;
    let collapseEnd = null;
    ...
    for (let i2 = 1; i2 < view.state.doc.lines; i2++) {
      ...
      if (collapseStart && collapseEnd) {
        if (folded)
          view.dispatch({ effects: uncollapse.of((from, to) => {
            return to <= collapseStart.from || from >= collapseEnd.to;
          }) });
        else
...
function getCharWidth(state, default_value) {
  let charWidths = Array.from(state.field(import_obsidian2.editorEditorField).contentDOM.querySelectorAll(".HyperMD-codeblock-end")).reduce((result, beginningElement) => {
    let nextElement = beginningElement.previousElementSibling;
    if (!nextElement)
      return result;
    let lineNumberElement = nextElement.querySelector("[class^='code-styler-line-number']");
...
async function readingViewPostProcessor(element, { sourcePath, getSectionInfo, frontmatter }, plugin, editingEmbeds = false) {
  var _a;
  const cache = plugin.app.metadataCache.getCache(sourcePath);
  if (!sourcePath || !element || ((_a = frontmatter != null ? frontmatter : cache == null ? void 0 : cache.frontmatter) == null ? void 0 : _a["code-styler-ignore"]) === true)
    return;
  await sleep(50);
  ...
  if (codeblockSectionInfo && specific && !editingEmbeds)
    ...
    if (!(!editingEmbeds && element.classList.contains("admonition-content"))) {
      let contentEl = element.matchParent(".view-content");
      await readingViewPostProcessor(contentEl ? contentEl : element.matchParent("div.print"), { sourcePath, getSectionInfo, frontmatter }, plugin, editingEmbeds);
    }
  } else
...
async function remakeCodeblock(codeblockCodeElement, codeblockPreElement, codeblockParameters, dynamic, plugin) {
  ...
  if (dynamic) {
    headerContainer.addEventListener("click", () => {
      ...
      const executeCodeOutput = codeblockPreElement.querySelector("pre > code ~ code.language-output");
    ...
    setTimeout(() => {
      setCollapseStyling(codeblockPreElement, codeblockCodeElement, codeblockParameters.fold.enabled);
    }, PRIMARY_DELAY);
  } else if (codeblockParameters.fold.enabled)
...
function remeasureReadingView(element, primary_delay = PRIMARY_DELAY) {
  ...
  codeblockPreElements.forEach((codeblockPreElement) => {
    ...
    if (!codeblockCodeElement)
      return;
    setTimeout(() => {
      setCollapseStyling(codeblockPreElement, codeblockCodeElement, codeblockPreElement.classList.contains("code-styler-collapsed"));
    }, primary_delay);
  });
}
function setCollapseStyling(codeblockPreElement, codeblockCodeElement, fold) {
...
var readingStylingMutationObserver = new MutationObserver((mutations) => {
  mutations.forEach((mutation) => {
    if (mutation.addedNodes.length !== 0)
      mutation.addedNodes.forEach((addedNode) => remeasureReadingView(addedNode));
  });
});
var executeCodeMutationObserver = new MutationObserver((mutations) => {
  mutations.forEach((mutation) => {
    var _a, _b;
    if (mutation.type === "attributes" && mutation.attributeName === "style" && mutation.target.tagName === "CODE" && mutation.target.classList.contains("execute-code-output")) {
      const executeCodeOutput = mutation.target;
      if ((_a = executeCodeOutput.parentElement) == null ? void 0 : _a.classList.contains("code-styler-collapsed"))
        executeCodeOutput.style.maxHeight = "";
    } else if (mutation.type === "childList" && mutation.target.tagName === "CODE" && mutation.target.classList.contains("execute-code-output")) {
      const executeCodeOutput = mutation.target;
      setTimeout(() => {
    ...
    } else if (mutation.type === "attributes" && mutation.attributeName === "style" && mutation.target.tagName === "INPUT" && ((_b = mutation.target.parentElement) == null ? void 0 : _b.tagName) === "CODE") {
      const executeCodeOutput = mutation.target.parentElement;
      if (executeCodeOutput) {
        setTimeout(() => {
          executeCodeOutput.style.setProperty("--true-height", `calc(${executeCodeOutput.scrollHeight}px + 3.5 * var(--code-padding) + var(--header-separator-width)`);
        }, SECONDARY_DELAY);
      }
    } else if (mutation.type === "childList" && mutation.target.tagName === "PRE") {
...
var CodeStylerPlugin = class extends import_obsidian4.Plugin {
  async onload() {
    ...
    this.readingStylingMutationObserver = readingStylingMutationObserver;
    this.executeCodeMutationObserver = executeCodeMutationObserver;
    this.registerMarkdownPostProcessor(async (element, context) => {
      await readingViewPostProcessor(element, context, this);
    });
    this.registerEditorExtension(createCodeMirrorExtensions(this.settings, this.languageIcons));
