...
var SettingsTab = class extends import_obsidian.PluginSettingTab {
  ...
  createPickr(plugin, containerEl, setting, id, getRelevantThemeColour, saveRelevantThemeColour, disabled) {
    let pickr = new PickrResettable(plugin, containerEl, setting, getRelevantThemeColour, saveRelevantThemeColour);
...
function createCodeblockCodeMirrorExtensions(settings, languageIcons) {
  const collapseEffect = import_state.StateEffect.define();
  const uncollapseEffect = import_state.StateEffect.define();
  const temporaryUncollapseAnnotation = import_state.Annotation.define();
  const codeblockCharWidthStateField = import_state.StateField.define({
    create(state) {
  ...
  const codeblockLinesStateField = import_view.ViewPlugin.fromClass(
    ...
    },
    {
      ...
      provide: (plugin) => import_view.EditorView.atomicRanges.of((view) => view.state.field(codeblockCollapseStateField) || import_view.Decoration.none)
    }
  );
  const codeblockHeaderStateField = import_state.StateField.define({
    create(state) {
  ...
  const codeblockCollapseStateField = import_state.StateField.define({
    create(state) {
      var _a;
      if (editingViewIgnore(state))
        return import_view.Decoration.none;
      const builder = new import_state.RangeSetBuilder();
      let codeblockHeaderDecorationSet = (_a = state.field(codeblockHeaderStateField, false)) != null ? _a : import_view.Decoration.none;
      for (let iter = codeblockHeaderDecorationSet.iter(); iter.value !== null; iter.next()) {
        if (iter.value.spec.defaultFold)
          builder.add(iter.value.spec.codeblockStart, iter.value.spec.codeblockEnd, import_view.Decoration.replace({ effect: collapseEffect.of(import_view.Decoration.replace({ block: true }).range(iter.value.spec.codeblockStart, iter.value.spec.codeblockEnd)), block: true, inclusive: true }));
      }
  ...
  const codeblockTemporaryUncollapseStateField = import_state.StateField.define({
    create(state) {
      return import_view.Decoration.none;
    },
    update(value, transaction) {
  ...
  const inlineCodeStateField = import_view.ViewPlugin.fromClass(
    class InlineCodeDecoration {
      ...
      update(update) {
        if (update.docChanged || update.viewportChanged || update.selectionSet || update.state.field(import_obsidian2.editorLivePreviewField) !== update.startState.field(import_obsidian2.editorLivePreviewField)) {
          ...
          (0, import_language.syntaxTree)(view.state).iterate({
            ...
            enter: (syntaxNode) => {
              ...
              if (!parameters) {
                ...
                this.decorations = this.decorations.update({ add: modeHighlight({ start: syntaxNode.from + endOfParameters, text, language: parameters.language }) });
              }
            }
          });
        }
  ...
  function inCollapsedTransactionFilter() {
    return import_state.EditorState.transactionFilter.of((transaction) => {
      ...
      let collapsedRangeSet = (_a = transaction.startState.field(codeblockCollapse, false)) != null ? _a : import_view.Decoration.none;
      let temporarilyUncollapsedRangeSet = (_b = transaction.startState.field(temporarilyUncollapsed, false)) != null ? _b : import_view.Decoration.none;
      transaction.newSelection.ranges.forEach((range) => {
        ...
        for (let iter = codeblockTemporaryUncollapseDecorationSet.map(transaction.changes).iter(); iter.value !== null; iter.next()) {
          if (!(iter.from <= range.head && range.head <= iter.to))
            extraTransactions.push({ effects: collapseEffect.of(import_view.Decoration.replace({ block: true }).range(iter.from, iter.to)), annotations: temporaryUncollapseAnnotation.of({ decorationRange: { from: iter.from, to: iter.to, value: iter.value }, uncollapse: false }) });
        }
      });
      ...
      if (currentDelimiter) {
        if (!startLine) {
          ...
          if (currentDelimiter === startDelimiter) {
            if (addHeader)
              builder.add(startLine.from, startLine.from, import_view.Decoration.widget({ widget: new HeaderWidget(codeblockParameters, settings.currentTheme.settings, languageIcons), block: true, side: -1, codeblockStart: startLine.from, codeblockEnd: line.to, defaultFold: codeblockParameters.fold.enabled }));
            startLine = null;
          }
        }
      }
  ...
  function collapseOnClick(view, target) {
    ...
    let codeblockHeaderDecorationsSet = (_a = view.state.field(codeblockHeaderStateField, false)) != null ? _a : import_view.Decoration.none;
    for (let iter = codeblockHeaderDecorationsSet.iter(); iter.value !== null; iter.next()) {
      if (position === iter.value.spec.codeblockStart) {
        let fold = true;
        (_b = view.state.field(codeblockCollapseStateField, false)) == null ? void 0 : _b.between(position, position, () => {
          fold = false;
        });
        if (fold)
          view.dispatch({ effects: collapseEffect.of(import_view.Decoration.replace({ block: true }).range(iter.value.spec.codeblockStart, iter.value.spec.codeblockEnd)) });
        else
          view.dispatch({ effects: uncollapseEffect.of({ filter: (from, to) => to <= iter.value.spec.codeblockStart || from >= iter.value.spec.codeblockEnd, filterFrom: iter.value.spec.codeblockStart, filterTo: iter.value.spec.codeblockEnd }) });
        view.requestMeasure();
      }
    }
  }
  function collapseAll(view, fold) {
    var _a;
    let codeblockHeaderDecorationsSet = (_a = view.state.field(codeblockHeaderStateField, false)) != null ? _a : import_view.Decoration.none;
    for (let iter = codeblockHeaderDecorationsSet.iter(); iter.value !== null; iter.next()) {
      let toFold;
...
function modeHighlight({ start, text, language }) {
  var _a;
  let markDecorations = [];
...
async function readingViewCodeblockDecoratingPostProcessor(element, { sourcePath, getSectionInfo, frontmatter }, plugin, editingEmbeds = false) {
  var _a;
  ...
  if (codeblockSectionInfo && specific && !editingEmbeds)
    ...
    if (!(!editingEmbeds && element.classList.contains("admonition-content"))) {
      ...
      await readingViewCodeblockDecoratingPostProcessor(contentEl ? contentEl : element.matchParent("div.print"), { sourcePath, getSectionInfo, frontmatter }, plugin, editingEmbeds);
    }
  } else
...
async function readingViewInlineDecoratingPostProcessor(element, { sourcePath, getSectionInfo, frontmatter }, plugin) {
  ...
  for (let inlineCodeElement of Array.from(element.querySelectorAll(":not(pre) > code"))) {
    ...
    let renderedCodeElement;
    let inlineCodeText = inlineCodeElement.innerText;
    let { parameters, text } = parseInlineCode(inlineCodeText);
    if (!parameters) {
      if (!text)
        return;
      import_obsidian3.MarkdownRenderer.renderMarkdown("`".repeat(20) + text + "`".repeat(20), tempRenderContainer, "", new import_obsidian3.Component());
...
var CodeStylerPlugin = class extends import_obsidian4.Plugin {
  async onload() {
    ...
    this.registerEditorExtension(codemirrorExtensions);
    ...
    this.addCommand({ id: "fold-all", name: "Fold all codeblocks", callback: () => {
      ...
      if (activeView) {
        if (activeView.getMode() === "preview")
          activeView.contentEl.querySelectorAll(".markdown-reading-view .code-styler-collapsed").forEach((headerElement) => {
            headerElement.classList.remove("code-styler-collapsed");
          });
        else
